...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_UP)
	{
		m_glRenderer.aXY += 0.1;
		if (m_glRenderer.aXY > 1.5)
			m_glRenderer.aXY = 1.5;
		Invalidate();
	}
	else if (nChar == VK_DOWN)
	{
		m_glRenderer.aXY -= 0.1;
		if (m_glRenderer.aXY < 0.1)
			m_glRenderer.aXY = 0.1;
		Invalidate();
	}
	else if (nChar == VK_LEFT)
	{
		m_glRenderer.aXZ += 0.1f;
		Invalidate();
	}
	...
	else if (nChar == VK_ADD || nChar == '+')
	{
		m_glRenderer.eyeR -= 5;
		Invalidate();
	}
	else if (nChar == VK_SUBTRACT || nChar == '-')
	{
		m_glRenderer.eyeR += 5;
		Invalidate();
	}
	...
	{
		m_glRenderer.axes = !m_glRenderer.axes;
		Invalidate();
	}
