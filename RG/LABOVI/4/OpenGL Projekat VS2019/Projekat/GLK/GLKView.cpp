...
void CGLKView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (nFlags && MK_LBUTTON)
	{
		...
		m_glRenderer.angleXY -= toRad(delta.y) * 10;
		m_glRenderer.angleXZ += toRad(delta.x) * 10;
...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	if (nChar == VK_SPACE)
	{
		m_glRenderer.drawGrid = !m_glRenderer.drawGrid;
		Invalidate();
	}
	else if (nChar == VK_UP)
	{
		m_glRenderer.angleXY += PI;
		Invalidate();
	}
