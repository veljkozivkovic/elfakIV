...
void CGLKView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	{
		m_glRenderer.eyeR += 5;
		if (m_glRenderer.eyeR > 50)
		{
			m_glRenderer.eyeR = 50;
	...
	{
		m_glRenderer.axis = !m_glRenderer.axis;
		Invalidate();
	}
	else if (nChar == 'L')
	{
...
void CGLKView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (nFlags && MK_LBUTTON)
	{
		...
		m_glRenderer.aXY -= 2 * toRad(delta.y);
		m_glRenderer.aXZ += 2 * toRad(delta.x);
		Invalidate();
	}
	prev = point;
